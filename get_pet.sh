...
cdo expr,'es = (esTmax + esTmin)/2' -selvar,esTmax,esTmin "${st}esTmax.nc" "${st}esTmin.nc" "${st}es.nc"
cdo expr,'vpd = es*(1 - hurs/100)' -selvar,es,hurs "${st}es.nc" $input_file_hurs "${st}vpd.nc"
